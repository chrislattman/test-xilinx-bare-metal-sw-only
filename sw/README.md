...
To build the project non-interactively, run `vitis -s build.py` from the `sw` folder. To build the BOOT.bin file for QSPI flash or QEMU, run `bootgen -arch zynq -image system.bif -o BOOT.bin -w` from the `sw` folder.
